...
GitHub Pages serves the static export, while the Docker image runs the same static export behind a tiny Bun server that also listens on `/api/reactions`. When the site can reach that endpoint it augments the static index with model-predicted reactions (and asks the model to enumerate every distinct balanced form); when it cannot (e.g. on GitHub Pages), it falls back to the static index alone. The server caches model responses on disk (`REACTION_CACHE_DIR`), so repeated requests for the same species never hit the model provider twice.
...
`devops/nix/docker-compose.yml` is generated from `devops/nix/arion-compose.nix`:
