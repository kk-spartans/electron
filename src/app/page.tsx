...
function AtomLearning({atom,atoms,bonds}:{atom:AtomNode;atoms:AtomNode[];bonds:BondEdge[]}) {
  ...
  const signature=Object.keys(counts).sort().map((symbol)=>symbol+(counts[symbol]>1?counts[symbol]:"")).join("");
  const stable=signature==="LiO2"?{tone:"good",title:"RDKit-valid superoxide",text:"The O₂⁻ unit is a radical anion. Its −1 charge and odd electron are delocalized over both oxygen atoms."}:connected.length===0?{tone:"neutral",title:"Unbonded",text:"Move the atom near compatible partners to test a structure."}:shellCount===shellTarget||Boolean(exception)?{tone:"good",title:"Locally satisfied",text:isIonic?`After electron transfer, shell ${outerShell} is the ion’s outer occupied shell and contains ${shellCount} of ${shellTarget} electrons.`:`The displayed valence shell has ${shellCount} electrons when shared electrons are counted.`}:shellCount<shellTarget?{tone:"warn",title:"Incomplete valence shell",text:`The actual outer occupied shell ${isIonic?`(shell ${outerShell}) `:""}contains ${shellCount} of ${shellTarget} electrons.`}:{tone:"warn",title:"Check this structure",text:`The actual outer occupied shell contains ${shellCount} electrons, above its usual capacity of ${shellTarget}.`};
