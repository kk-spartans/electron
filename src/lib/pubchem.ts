...
export type StructureRecord = {
  ...
  formula: string;
  source: string;
  atoms: Array<{ aid: number; atomicNumber: number; x: number; y: number }>;
...
export async function lookupStructure(query: string): Promise<StructureResult> {
  ...
  try {
    ...
    const formula =
      formulaTokens.map((match) => match[0]).join("") === compact &&
      formulaTokens.every((match) => elementSymbols.has(match[1]))
        ? compact
        : "";
    ...
    if (cidMatch) cid = Number(cidMatch[1]);
      ...
      if (matches.length > 1) {
        ...
        return {
          error: "Choose the PubChem structure you mean.",
          candidates: (candidates.PropertyTable?.Properties ?? []).map((item) => ({
            cid: item.CID,
            name: item.Title ?? `PubChem compound ${item.CID}`,
    ...
    const data = (await pubchem(`compound/cid/${cid}/JSON?record_type=2d`)) as {
      PC_Compounds?: PubChemRecord[];
    };
    const record = data.PC_Compounds?.[0],
      coordinateSet = record?.coords?.[0],
      conformer = coordinateSet?.conformers?.[0];
    const aids = record?.atoms?.aid ?? [],
      atomicNumbers = record?.atoms?.element ?? [],
      coordinateAids = coordinateSet?.aid ?? [];
    if (!record || !aids.length || !conformer?.x || !conformer.y)
      throw new Error("PubChem returned no drawable conformer");
    ...
    const atoms = aids.map((aid, index) => {
      ...
      return {
        aid,
        atomicNumber: atomicNumbers[index],
        x: conformer.x![position],
        y: conformer.y![position],
      };
    });
    const bonds = (record.bonds?.aid1 ?? []).map((from, index) => ({
      from,
      to: record.bonds!.aid2![index],
      order: Math.max(1, Math.min(3, record.bonds!.order?.[index] ?? 1)),
    }));
    ...
    return { error: error instanceof Error ? error.message : "The structure could not be loaded." };
  }
}
